...
namespace Examples.TagHelpers
{
    ...
    public class MovieTagHelper:TagHelper
    {
        ...
        private TagBuilder GetTitle()
        {
            ...
            year.InnerHtml.AppendHtml(string.Format("({0})", this.ReleaseYear.Model));

            var title = new TagBuilder("div");
            title.Attributes.Add("class", "movie-title");
            title.InnerHtml.AppendHtml(string.Format("{0}", this.Title.Model));
        ...
        private TagBuilder GetDirector()
        {
            ...
            director.InnerHtml.AppendHtml(string.Format("<span>Director:{0}</span>", this.Director.Model));
            return director;
        }
        ...
        private TagBuilder GetSummary()
        {
            ...
            summary.InnerHtml.AppendHtml(string.Format("<span><strong>Plot:</strong>{0}</span>",this.Summary.Model));
            return summary;
        }
